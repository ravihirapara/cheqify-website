...
1. Open **Bing Webmaster Tools** → cheqify.app property → **URL Submission**
2. Pick the next un-checked Batch below
3. Paste the URLs (10 at a time) → click **Submit URLs**
4. Mark the batch complete `[x]` below
5. If quota hits mid-day, stop and resume tomorrow
6. Bing's quota for new properties is generous — you can usually submit all batches in one sitting
...
- [x] Sitemap submitted (`https://cheqify.app/sitemap.xml`) — done 2026-05-08, 93 URLs discovered
- [ ] IndexNow page in BWT shows recent submissions (proves auto-ping pipeline is working)
- [ ] cheqify.app/en returns 200 (verified post Netlify plugin fix on 2026-05-08)
...
## Batch 9 — HI blog posts, part 2 (4 URLs, short batch)
...
## Batch 10 — GU blog posts, part 1 (10 URLs)
...
## Batch 11 — GU blog posts, part 2 (4 URLs, final batch)
...
## Skipped — auto-handled by IndexNow

These are **not** listed above because they auto-ping Bing the moment they're published in Sanity (via the IndexNow Netlify Function). No manual submission needed:

- All `/blog/<post-slug>` URLs technically already pinged via IndexNow on initial publish — but manual submission gives a stronger priority signal, so they ARE included in Batches 6–11 above for the new-domain crawl push.

## Skipped — low SEO priority

```
# Privacy/Terms — low search volume, auto-discovered via sitemap
...
1. **Wait 24–48 hours**, then check **BWT → IndexNow** page to confirm submissions are landing.
2. **Wait 7–14 days**, then check **BWT → URL Inspection** on a few priority URLs:
   - If status changed from "Discovered but not crawled" → "Indexed" → ✅ working
   - If still "Discovered but not crawled" after 14 days → may need backlinks; manual submission alone can't fully overcome a new-domain trust deficit
3. **Wait 2–4 weeks** for **Search Performance** dashboard to populate with impressions/clicks. Bing reporting lag is longer than Google's.

---

## Related files

- `_bmad-output/seo/gsc-indexing-queue.md` — Google equivalent (7 URLs/day, 11-day plan)
- `_bmad-output/seo/SEO-PLAN.md` — overall SEO roadmap
- `_bmad-output/seo/rank-page-1-steps.md` — page-1 ranking action plan
