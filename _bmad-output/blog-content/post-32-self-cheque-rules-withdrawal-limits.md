...
Mechanically it's a [bearer cheque](https://cheqify.app/en/blog/cheque-types-india) whose payee happens to be the account holder. That bearer nature is the whole story, both its convenience and its danger, and we'll come back to it.

Writing one correctly takes ten seconds, and the [usual fill rules](https://cheqify.app/en/blog/how-to-fill-cheque-correctly) apply with extra strictness because a counter clerk will scrutinise it while you stand there:
...
- **Do NOT cross it.** This one trips people constantly: two parallel lines mean "account credit only," which makes cash payment impossible ([crossing, explained](https://cheqify.app/en/blog/crossed-cheque-vs-account-payee)). A crossed self cheque is a contradiction the bank resolves by refusing you cash.
...
- **Record every self cheque like any other issued cheque:** leaf number, date, amount, purpose. On the counterfoil and in your register. A self cheque that's just *missing* from your book is indistinguishable from a stolen leaf ([why the lifecycle audit trail matters](https://cheqify.app/en/blog/cheque-lifecycle-management-explained)).
...
Mechanically ये एक [bearer cheque](https://cheqify.app/hi/blog/cheque-types-india) है जिसका payee account holder खुद है. वो bearer nature पूरी story है, इसकी convenience भी और danger भी, और हम इस पर वापस आएँगे.

एक correctly लिखने में दस seconds लगते हैं, और [usual fill rules](https://cheqify.app/hi/blog/how-to-fill-cheque-correctly) extra strictness से apply होते हैं क्योंकि एक counter clerk इसे scrutinise करेगा जब आप वहाँ खड़े हैं:
...
- **इसे cross मत करें.** ये लोगों को constantly trip करता है: दो parallel lines का मतलब "account credit only," जो cash payment को impossible बना देता है ([crossing, explained](https://cheqify.app/hi/blog/crossed-cheque-vs-account-payee)). एक crossed self cheque एक contradiction है जिसे bank आपको cash refuse करके resolve करता है.
...
Weekly wage cash निकालने वाले एक SMB के लिए, वो ₹20 lakh non-filer trigger जितना लगता है उससे closer है: ₹40,000 per week साल के अंदर इसे cross कर जाता है. Returns file करें, aggregate देखें, और जहाँ vendors accept करें, cash की जगह [account-payee cheque](https://cheqify.app/hi/blog/crossed-cheque-vs-account-payee) या transfer से pay करें — जो cash आप कभी निकालते ही नहीं वो TDS है जो आप कभी reconcile नहीं करते.
...
- **हर self cheque को किसी भी दूसरे issued cheque की तरह record करें:** leaf number, date, amount, purpose. Counterfoil पर और आपके register में. एक self cheque जो आपकी book से बस *missing* है वो एक stolen leaf से indistinguishable है ([lifecycle audit trail क्यों matter करता है](https://cheqify.app/hi/blog/cheque-lifecycle-management-explained)).
...
Mechanically આ એક [bearer cheque](https://cheqify.app/gu/blog/cheque-types-india) છે જેનો payee account holder ખુદ છે. એ bearer nature આખી story છે, એની convenience પણ અને danger પણ, અને અમે આના પર પાછા આવીશું.

એક correctly લખવામાં દસ seconds લાગે છે, અને [usual fill rules](https://cheqify.app/gu/blog/how-to-fill-cheque-correctly) extra strictness થી apply થાય છે કારણ કે એક counter clerk એને scrutinise કરશે જ્યારે તમે ત્યાં ઊભા છો:
...
- **એને cross ન કરો.** આ લોકોને constantly trip કરે છે: બે parallel lines નો મતલબ "account credit only," જે cash payment ને impossible બનાવી દે છે ([crossing, explained](https://cheqify.app/gu/blog/crossed-cheque-vs-account-payee)). એક crossed self cheque એક contradiction છે જેને bank તમને cash refuse કરીને resolve કરે છે.
...
- **દરેક self cheque ને કોઈ પણ બીજા issued cheque ની જેમ record કરો:** leaf number, date, amount, purpose. Counterfoil પર અને તમારા register માં. એક self cheque જે તમારી book માંથી બસ *missing* છે એ એક stolen leaf થી indistinguishable છે ([lifecycle audit trail કેમ matter કરે છે](https://cheqify.app/gu/blog/cheque-lifecycle-management-explained)).
