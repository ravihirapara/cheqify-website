...
- **New joiners whose account details aren't verified yet** — the first salary often moves by cheque while the [cancelled-cheque-and-penny-drop onboarding](/en/blog/how-to-write-cancelled-cheque-safely) completes.
...
2. **Print the batch** — every cheque account-payee crossed, payee names exactly as bank records know them, amounts in words generated, dates uniform ([bulk printing, the full method](/en/blog/bulk-cheque-printing-batch-guide-india)).
3. **Register every leaf** — cheque number against employee name in the wage register and the [cheque register](/en/blog/cheque-lifecycle-management-explained), before distribution.
...
5. **Watch the float clear** — salary cheques present within days; reconcile cleared leaves and chase the stragglers (an undeposited salary cheque ages toward [its 3-month expiry](/en/blog/cheque-validity-period-stop-payment-india), and a lapsed one means reissue paperwork).
...
Payment of Wages Act सिर्फ़ *कैसे* regulate नहीं करता — *कब* भी करता है. 1,000 से कम employees वाले establishments को wages **अगले महीने की 7th तक** pay करनी होंगी; बड़ों को 10th तक. एक cheque payroll को calendar plus clearing respect करना होता है: 7th को handed over एक cheque जो [9th को clear होता है](/hi/blog/cheque-clearing-time-india) वो conversation है जो आप labour inspector से नहीं चाहते.

Clean pattern: salary cheques **महीने के last working day** पर print और distribute करें, ताकि slowest clearing भी window के well inside land हो. Employees अपनी convenience पर deposit करते हैं ([deposit playbook जो वो use करेंगे](/hi/blog/how-to-deposit-cheque-india)) और wage register payment time पर initiated दिखाता है.
...
- **नए joiners जिनके account details अभी verified नहीं** — पहली salary अक्सर cheque से move होती है जब तक [cancelled-cheque-and-penny-drop onboarding](/hi/blog/how-to-write-cancelled-cheque-safely) complete होती है.
...
2. **Batch print करें** — हर cheque account-payee crossed, payee names exactly जैसे bank records जानते हैं, amounts in words generated, dates uniform ([bulk printing, full method](/hi/blog/bulk-cheque-printing-batch-guide-india)).
3. **हर leaf register करें** — wage register और [cheque register](/hi/blog/cheque-lifecycle-management-explained) में employee name के against cheque number, distribution से पहले.
...
5. **Float को clear होते देखें** — salary cheques days के अंदर present होते हैं; cleared leaves reconcile करें और stragglers chase करें (एक undeposited salary cheque [अपनी 3-month expiry](/hi/blog/cheque-validity-period-stop-payment-india) की तरफ़ age होता है, और एक lapsed मतलब reissue paperwork).
...
एक bounced vendor cheque एक commercial dispute है. एक bounced *salary* cheque हर axis पर worse है: ये किसी भी dishonoured debt payment की तरह एक [Section 138 offence](/hi/blog/section-138-ni-act-cheque-bounce-india) है — owed salary जितना real debt हो सकता है उतना है — *plus* timing पर एक Payment of Wages Act violation, *plus* उस तरह का workforce-trust damage जो कोई apology पूरी तरह repair नहीं करती. Authorities wage non-payment को seriously लेती हैं, और return memo पकड़ा employee हर remedy एक साथ पकड़े है.
...
- **નવા joiners જેમના account details હજુ verified નથી** — પહેલી salary ઘણીવાર cheque થી move થાય છે જ્યાં સુધી [cancelled-cheque-and-penny-drop onboarding](/gu/blog/how-to-write-cancelled-cheque-safely) complete થાય છે.
...
2. **Batch print કરો** — દરેક cheque account-payee crossed, payee names exactly જેમ bank records જાણે છે, amounts in words generated, dates uniform ([bulk printing, full method](/gu/blog/bulk-cheque-printing-batch-guide-india)).
3. **દરેક leaf register કરો** — wage register અને [cheque register](/gu/blog/cheque-lifecycle-management-explained) માં employee name ની against cheque number, distribution પહેલા.
...
5. **Float ને clear થતું જુઓ** — salary cheques days ની અંદર present થાય છે; cleared leaves reconcile કરો અને stragglers chase કરો (એક undeposited salary cheque [પોતાની 3-month expiry](/gu/blog/cheque-validity-period-stop-payment-india) તરફ age થાય છે, અને એક lapsed મતલબ reissue paperwork).
