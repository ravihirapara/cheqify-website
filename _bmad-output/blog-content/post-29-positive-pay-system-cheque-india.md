...
When the cheque later arrives through clearing, the [Cheque Truncation System](/en/blog/cheque-truncation-system-explained) cross-verifies the presented cheque against the details you registered. Match: the cheque sails through normal processing. Mismatch: it gets flagged and referred back — the discrepancy is caught *before* money moves, not after.
...
- **Wrong cheque number** — adjacent leaf's number entered. Mismatch ([where the cheque number lives](/en/blog/cheque-number-in-cheque-book-explained)).
...
- **Keep the register and the cheque in one system.** Your [cheque lifecycle records](/en/blog/cheque-lifecycle-management-explained) already hold the number, date, payee, and amount of every leaf — Positive Pay registration is reading those fields out, not recreating them.
...
- **High-value cheques deserve the full discipline anyway:** account-payee crossing, no corrections on the leaf, dark permanent ink — Positive Pay backs up those habits ([the full prevention stack](/en/blog/how-to-avoid-cheque-fraud-in-india)); it doesn't replace them.
...
- **गलत cheque number** — adjacent leaf का number entered. Mismatch ([cheque number कहाँ रहता है](/hi/blog/cheque-number-in-cheque-book-explained)).
...
- **Register और cheque एक system में रखें.** आपके [cheque lifecycle records](/hi/blog/cheque-lifecycle-management-explained) already हर leaf का number, date, payee, और amount hold करते हैं — Positive Pay registration उन fields को पढ़ना है, recreate करना नहीं.
...
- **High-value cheques वैसे भी full discipline deserve करते हैं:** account-payee crossing, leaf पर कोई corrections नहीं, dark permanent ink — Positive Pay उन habits को back up करता है ([full prevention stack](/hi/blog/how-to-avoid-cheque-fraud-in-india)); replace नहीं करता.
...
જ્યારે cheque પછી clearing ના through પહોંચે છે, [Cheque Truncation System](/gu/blog/cheque-truncation-system-explained) presented cheque ને તમારી registered details ની against cross-verify કરે છે. Match: cheque normal processing થી sail કરી જાય છે. Mismatch: આ flagged થાય છે અને refer back — discrepancy પૈસા move થયા *પહેલા* પકડાય છે, પછી નહીં.
...
- **ખોટો cheque number** — adjacent leaf નો number entered. Mismatch ([cheque number ક્યાં રહે છે](/gu/blog/cheque-number-in-cheque-book-explained)).
...
- **Register અને cheque એક system માં રાખો.** તમારા [cheque lifecycle records](/gu/blog/cheque-lifecycle-management-explained) already દરેક leaf નો number, date, payee, અને amount hold કરે છે — Positive Pay registration એ fields ને વાંચવાનું છે, recreate કરવાનું નહીં.
...
- **High-value cheques એમ પણ full discipline deserve કરે છે:** account-payee crossing, leaf પર કોઈ corrections નહીં, dark permanent ink — Positive Pay એ habits ને back up કરે છે ([full prevention stack](/gu/blog/how-to-avoid-cheque-fraud-in-india)); replace નથી કરતું.
